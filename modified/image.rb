...
# image_nl imageにclass=nlを追加
#
# image_link( number, 'desc' ) - 画像へのリンクを生成します。
#    number - 画像の番号0、1、2等
#    desc - 画像の説明
#
# image_idx( filename, baseurl, 'altword', size, place ) - imageindexで作ったアルバムへのリンク
#
# その他:
...
def image_nl( id, alt = "image", thumbnail = nil, width = nil )
   image( id, alt, thumbnail, width, "nl" )
end
...
def image_idx(filename, baseurl, alt = "image", place = 'nl')
   #if size.kind_of?(Array)
   #   size = %Q| width="#{h size[0]}" height="#{h size[1]}"|
   #else
   #   size = %Q| width="#{size.to_i}"|
   #end
   %Q[<a href="#{h baseurl}/slides/#{h filename.sub(/\.(jpg|png|jpeg|gif|svg|mov)/i, '')}.html" class="albumlink" title="#{h alt}"><img src="#{h baseurl}/thumbnail/#{h filename}" class="#{place}" /></a>]
end

def image_cook(filename, baseurl = "http://kmuto.jp/d/photo")
   %Q[<a href="#{h baseurl}/full/#{h filename}" class="cooking"><img src="#{h baseurl}/#{h filename}" class="#{cooking}" /></a>]
end
